...
|-------|---------|-------------|
| `gost-report` | Russian academic reports (`.docx`) to GOST 7.32 — лабораторные, отчёты по практике, курсовые, ВКР. Built-in `ITMO_PROFILE` / `GOST_PROFILE` plus `UniversityProfile` for any other vuz. Opt-in plugin modules: `r.plot.*` (matplotlib charts, GOST style) and `r.diagram` (Graphviz/DOT → PNG), sharing one figure counter. | «лабораторную», «отчёт по ГОСТ», «курсовую», «ИТМО», etc. |
