...
- **Opt-in personal preferences**: `--with-sound-hooks` (Stop sound hook only — one beep when Claude finishes a turn, OS auto-detected), `--with-notification-sound` (Notification sound hook only — beep on permission prompts and "waiting for input"), `--with-thinking-summaries` (`showThinkingSummaries: true`). Passing both sound flags is allowed but warns: Notification often fires right after Stop, producing two beeps in sequence. Use `--clean-sound-hooks` (`-CleanSoundHooks` on PowerShell) to strip every sound hook from `settings.json` without touching unrelated hooks (gost-validation, user customs).
